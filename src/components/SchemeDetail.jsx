...
const DocPreview = ({ docKey, scheme }) => {
  ...
  if(docKey==="boq" || docKey==="boq_summary"){
    ...
    // Mini cost-bar mirror of the on-screen Summary, only shown for the PDF preview.
                ...
                {isWorking ? (
                  <button className="btn sm ghost" style={{marginLeft:"auto"}} onClick={()=> {
                    ...
                    if (d.key === 'boq_summary') {
                      // Compile the PDF straight from the live scheme state — works
                      // even if the designer hasn't opened the BoQ tab in this session.
                      const E = window.BOQ_ENGINE;
                    ...
                    return onPreview(scheme, d.key);
                  }}>
                    {d.key==='boq' ? 'Open BoQ' : d.key==='boq_summary' ? 'Download PDF' : 'Preview'} <Icon.Arrow />
                  </button>
                ) : (
