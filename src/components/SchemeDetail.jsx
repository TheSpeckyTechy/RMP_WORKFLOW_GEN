...
// ─── Traffic Management placeholder (used when no TM PDFs uploaded) ──────────

const TmPlaceholderDoc = ({ scheme }) => {
  const contractor = scheme.contractor || 'Tayside Contracts';
  return (
    <div style={{width:794,minHeight:1123,background:'white',fontFamily:"'Arial',Helvetica,sans-serif",color:'#111',boxSizing:'border-box',display:'flex',flexDirection:'column'}}>
      {/* Letterhead — matches Front Sheet */}
      <div style={{background:'#1a3a5c',color:'white',padding:'28px 40px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <div>
          <div style={{fontSize:10,letterSpacing:'0.15em',textTransform:'uppercase',opacity:0.7,marginBottom:4}}>Dundee City Council</div>
          <div style={{fontSize:20,fontWeight:700,letterSpacing:'-0.01em'}}>Road Maintenance Partnership</div>
      ...
      {/* Title band */}
      <div style={{background:'#f0f4f8',borderBottom:'3px solid #1a3a5c',padding:'20px 40px'}}>
        <div style={{fontSize:20,fontWeight:800,lineHeight:1.2,marginBottom:4}}>Traffic Management Plans</div>
        <div style={{fontSize:13,color:'#555',marginBottom:8}}>{scheme.road_name || 'Untitled Scheme'}{scheme.scheme_extent ? ` — ${scheme.scheme_extent}` : ''}</div>
        <div style={{display:'flex',gap:24,flexWrap:'wrap',fontSize:12,marginTop:6}}>
          {[["Project Ref",scheme.project_number],["TM Type",scheme.tm_type],["Working Hours",scheme.tm_hours]].map(([l,v])=>(
            <div key={l}><span style={{color:'#888',marginRight:6}}>{l}</span><span style={{fontWeight:600,fontFamily:l==='Project Ref'?'monospace':'inherit'}}>{v||'—'}</span></div>
          ))}
        </div>
      </div>
      ...
      {/* Notice band */}
      <div style={{background:'#f59e0b',color:'white',padding:'12px 40px',display:'flex',gap:16,alignItems:'center',fontSize:12}}>
        <div style={{fontSize:9,textTransform:'uppercase',letterSpacing:'0.1em',opacity:0.9}}>Notice</div>
        <div style={{fontWeight:700}}>Traffic Management to be delivered by Subcontractor</div>
      </div>

      {/* Body */}
      <div style={{padding:'40px 40px 28px',flex:1,display:'flex',flexDirection:'column',gap:18}}>
        <div style={{fontSize:9,fontWeight:700,textTransform:'uppercase',letterSpacing:'0.1em',color:'#888',paddingBottom:6,borderBottom:'2px solid #1a3a5c'}}>Statement</div>
        <div style={{fontSize:13,lineHeight:1.6,color:'#222'}}>
          Traffic management for this scheme will be delivered by the appointed subcontractor.
          Traffic management drawings will be supplied to {contractor} by the subcontractor in advance of the works
          and issued under separate cover. They are therefore not included in this handover pack at this stage.
        </div>
        <div style={{fontSize:12,lineHeight:1.6,color:'#444'}}>
          On receipt, the drawings will be appended to this section of the handover pack and reissued as required.
          Site setup, signage and operation will conform to Chapter 8 of the Traffic Signs Manual and the
          Safety at Street Works and Road Works Code of Practice.
        </div>

        <div style={{marginTop:8,padding:'14px 16px',background:'#f8f9fa',border:'1px solid #e5e7eb',borderRadius:5,fontSize:12,color:'#444'}}>
          <div style={{fontSize:9,fontWeight:700,textTransform:'uppercase',letterSpacing:'0.1em',color:'#888',marginBottom:8}}>Scheme Reference</div>
          <div style={{display:'grid',gridTemplateColumns:'140px 1fr',rowGap:6,columnGap:10}}>
            <div style={{color:'#888'}}>Project Number</div><div style={{fontFamily:'monospace',fontWeight:600}}>{scheme.project_number || '—'}</div>
            <div style={{color:'#888'}}>Road</div><div style={{fontWeight:500}}>{scheme.road_name || '—'}</div>
            <div style={{color:'#888'}}>Contractor</div><div style={{fontWeight:500}}>{contractor}</div>
            <div style={{color:'#888'}}>TM Type</div><div style={{fontWeight:500}}>{scheme.tm_type || '—'}</div>
            <div style={{color:'#888'}}>Working Hours</div><div style={{fontWeight:500}}>{scheme.tm_hours || '—'}</div>
            <div style={{color:'#888'}}>Works Period</div><div style={{fontWeight:500}}>{scheme.date_start && scheme.date_finish ? `${scheme.date_start} → ${scheme.date_finish}` : (scheme.date_start || '—')}</div>
          </div>
        </div>
      </div>
...
window.TmPlaceholderDoc = TmPlaceholderDoc;

window.__getTmPlaceholderPdfBuffer = async (scheme) => {
  const container = document.createElement('div');
  container.style.cssText = 'position:fixed;left:-10000px;top:0;width:794px;background:white;';
  document.body.appendChild(container);
  try {
    const root = ReactDOM.createRoot(container);
    root.render(React.createElement(TmPlaceholderDoc, { scheme }));
    await new Promise(r => setTimeout(r, 500));
    const buf = await window.htmlToPdfBuffer(container.firstChild || container);
    root.unmount();
    return buf;
