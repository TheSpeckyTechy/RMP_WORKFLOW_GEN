...
const GenerateModal = ({ scheme, onClose }) => {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      // that pages be created in the document they're added to, and the
      // safest way to honour that is to do every addPage call on a single
      // final document at the very end (Pass 3). Page counts gathered here
      // also drive the front-sheet contents listing in Pass 2.
      // resolved[i] = { section, parts: [{ srcDoc, pageCount }] }
      const resolved = [];
      ...
      for (const section of NON_FRONT_DOCS) {
        ...
        try {
          ...
          if (parts.length > 0) {
            ...
            const fileNote = parts.length > 1 ? `${parts.length} files · ${totalPages}pp` : `${totalPages} page${totalPages !== 1 ? 's' : ''}`;
