// Dashboard view — reads from SchemeContext
    ...
    <>
      ...
      <div className="stats">
        <div className="stat">
          ...
          <div className="stat-meta"><span className="up">▲ 2</span> vs last quarter</div>
        </div>
        <div className="stat">
          <div className="stat-label">Combined tender value</div>
          <div className="stat-value">{fmtGBP(totals.tender)}</div>
          <div className="stat-meta">FY 2025/26 + 2026/27</div>
        </div>
        <div className="stat">
          <div className="stat-label">Total area</div>
          <div className="stat-value">{totals.m2.toLocaleString()}<span className="unit">m²</span></div>
          <div className="stat-meta">across {totals.live} roads</div>
        </div>
        <div className="stat">
          <div className="stat-label">Packs ready to issue</div>
          <div className="stat-value">{totals.ready}<span className="unit">/ {totals.live}</span></div>
          <div className="stat-meta">auto-generated from workbook</div>
        </div>
      </div>
        ...
        <table className="schemes">
          <thead>
            <tr>
              ...
              <th>Scheme</th>
              <th style={{ width: 120 }}>Ward</th>
              <th style={{ width: 170 }}>Treatment</th>
              <th style={{ width: 100 }}>Area</th>
              <th style={{ width: 120 }}>Tender</th>
              <th style={{ width: 160 }}>Pack</th>
            ...
            {list.map(s => {
              ...
              return (
                <tr key={s.id} onClick={() => onOpen(s.id)}>
                  ...
                  <td>
                    <div className="row-name">{s.road_name}</div>
                  ...
                  <td style={{ fontSize: 12, color: "var(--ink-2)" }}>
                    {s.treatment_type}
                    <div className="row-sub">{s.total_depth_mm}mm · {s.scheme_type}</div>
                  </td>
                  ...
                  <td>
                    <div className="pack-bar">
                      <div className="pack-bar-track">
                        <div className={"pack-bar-fill " + (pct === 100 ? "full" : "")} style={{ width: pct + "%" }}></div>
                      </div>
                      <div className="pack-bar-count">{s.packProgress}/{s.packTotal}</div>
                    </div>
                  </td>
