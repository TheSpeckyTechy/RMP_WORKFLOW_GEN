// Master Workbook tab — all 49 named ranges, 8 sections
    ...
    if (f.type === "calc") {
      return (
        <div className="mwb-row calc" key={f.key}>
          ...
          <div className="mwb-val calc-val">
            <span className={f.mono ? "mono" : ""}>{value}</span>
      ...
      return (
        <div className="mwb-row" key={f.key}>
          ...
          <div className="mwb-val">
            <select value={scheme.ward_num} onChange={e => {
    ...
    if (f.type === "select") {
      return (
        <div className="mwb-row" key={f.key}>
          ...
          <div className="mwb-label">{f.label}</div>
          <div className="mwb-val">
            <select value={value} onChange={e => update(f.key, e.target.value)}>
    ...
    if (f.type === "link") {
      return (
        <div className="mwb-row" key={f.key}>
          ...
          <div className="mwb-label">{f.label}</div>
          <div className="mwb-val">
            <span style={{ color: "var(--accent)", fontSize: 12 }}>🔗 {value || "Click to open DCC lookup"}</span>
          </div>
        </div>
      );
    }
        ...
        <div className="mwb-row" key={f.key}>
          ...
          <div className="mwb-val">
            <textarea
              ...
              style={{ width: "100%", fontFamily: "inherit", fontSize: 12, padding: "6px 8px", border: "1px solid var(--line)", borderRadius: 3, background: "var(--bg)", color: "var(--ink)", resize: "vertical", lineHeight: 1.45 }}
            />
          </div>
        </div>
      );
      ...
      <div className="mwb-row" key={f.key}>
        ...
        <div className="mwb-val">
          <input
            ...
            value={value}
            onChange={e => update(f.key, f.type === "number" ? (e.target.value === "" ? "" : +e.target.value) : e.target.value)}
          />
        </div>
      </div>
    );
    ...
    <div className="master-workbook">
      ...
      {window.WORKBOOK_SCHEMA.map(section => (
        <div className="mwb-section" key={section.section}>
          <div className="mwb-section-head">
            ...
            <span style={{ marginLeft: "auto", fontSize: 10, color: "var(--ink-3)", fontFamily: "var(--font-mono)", textTransform: "uppercase", letterSpacing: "0.06em" }}>
              {section.fields.length} fields
            </span>
          </div>
          <div className="mwb-grid">
            <div className="mwb-row header">
              <div className="mwb-key mono">named range</div>
              <div className="mwb-label">label</div>
              <div className="mwb-val">value</div>
            </div>
      ...
      <div className="mwb-foot">
        <div style={{ fontSize: 12, color: "var(--ink-2)" }}>
          <strong>How it works:</strong> Every value here is a named range in the master workbook. Downstream Word templates use Content Controls linked to these ranges — when you change a value here, regenerating the pack updates every document. No copy-paste.
        </div>
      </div>
    </div>
  );
